...
class AwsIamDetachRolePolicyV1
  ...
  def execute()
    ...
    resp = @iam.detach_role_policy(
      role_name: @parameters['role_name'],
      policy_arn: @parameters['policy_arn']
    )
    puts resp if @enable_debug_logging

    puts "Policy '#{@parameters['policy_arn']}' was detached from Role '#{@parameters['role_name']}'." if @enable_debug_logging
