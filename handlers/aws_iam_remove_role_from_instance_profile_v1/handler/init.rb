...
class AwsIamRemoveRoleFromInstanceProfileV1
  ...
  def execute()
    ...
    resp = @iam.remove_role_from_instance_profile(
      instance_profile_name: @parameters['instance_profile_name'],
      role_name: @parameters['role_name']
    )
    puts resp if @enable_debug_logging

    puts "Role '#{@parameters['role_name']}' was removed from instance profile '#{@parameters['instance_profile_name']}'." if @enable_debug_logging
